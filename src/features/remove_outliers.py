...
# Boxplots for the first three outlier columns grouped by label
df[outlier_columns[:3] + ["label"]].boxplot(by="label", figsize=(20, 10), layout=(1, 3))

# Boxplots for the remaining outlier columns grouped by label
df[outlier_columns[3:] + ["label"]].boxplot(by="label", figsize=(20, 10), layout=(1, 3))
...
#PROBLEM !! Doesn't diffrentiate between the different activities. Need to apply lables and then outliers

# --------------------------------------------------------------
# Chauvenets criteron (distribution based)
# --------------------------------------------------------------

# Check for normal distribution -> Need it !! Check for Bell Shapey
...
def mark_outliers_lof(dataset, columns, n=20):
    ...
    Basically checks one whole row with the 20 nearest to see if its an outlier or not ! 20 recommended standard 
...
#Marks outliers within the actual data itself 
...
# Loop through each column and label to process outliers
for col in outlier_columns:
    for label in df["label"].unique():
        # Apply the Chauvenet outlier detection method
        dataset = mark_outliers_chauvenet(df[df["label"] == label], col)
...
# --------------------------------------------------------------
outliers_removed_df.to_pickle("../../data/interim/02.outliers_removed_chauvenets.pkl")